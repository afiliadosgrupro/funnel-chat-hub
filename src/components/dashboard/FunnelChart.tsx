...
const FunnelChart = () => {
  ...
  const fetchLeadsData = async () => {
    ...
    try {
      ...
      const {
        data,
        error
      } = await supabase.from('Março | FUNIL Bruno').select('funil').gte('created_at', today.toISOString());
      if (error) {
        throw error;
      }
  ...
  const CustomTooltip = ({
    ...
    return <div className="rounded-md border bg-white p-2 shadow-md">
  ...
  return <Card>
      <CardHeader className="flex flex-row items-center justify-between pb-2 bg-emerald-200 mx-[240px] my-0 px-[240px] py-[2px] rounded-none">
        <CardTitle className="text-lg font-medium">Leads por Etapa (Hoje)</CardTitle>
        <button onClick={fetchLeadsData} className="text-gray-400 hover:text-gray-600" disabled={loading}>
      ...
      <CardContent className="h-[300px]">
        {loading ? <div className="flex items-center justify-center h-full">
          ...
          </div> : error ? <div className="flex items-center justify-center h-full text-red-500">
            {error}
          </div> : <ChartContainer config={chartConfig} className="h-full">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data}>
                <XAxis dataKey="label" tickLine={false} axisLine={false} fontSize={12} />
                <YAxis tickLine={false} axisLine={false} fontSize={12} allowDecimals={false} />
                <Tooltip content={<CustomTooltip />} />
                <Bar dataKey="count" fill="var(--color-Apresentação, #22c55e)" radius={[4, 4, 0, 0]} barSize={30} />
              </BarChart>
            </ResponsiveContainer>
          </ChartContainer>}
      </CardContent>
